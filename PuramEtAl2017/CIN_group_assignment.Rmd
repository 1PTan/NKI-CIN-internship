...
```{r}
group_assignment <- CNV_metrics %>% filter(str_detect(group, "tumor")) %>% mutate(CIN_status = ifelse(heterogeneity_score > 0.25 & aneuploidy_score > 0.035, "CIN_high", "CIN_low"))
...
```{r}
group_assignment_medf <- CNV_metrics_medf %>% filter(str_detect(group, "tumor")) %>% mutate(CIN_status = ifelse(heterogeneity_score > 0.011 & aneuploidy_score > 0.025, "CIN_high", "CIN_low"))
