...
```{r}
annot_scRNA <- read.table("cellAnnotations.txt")
colnames(annot_scRNA) <- c("sample_id", "annotation")
...
filter <- table(annot_scRNA$annotation) %>% as.data.frame() %>% mutate_if(is.factor, as.character) %>% filter(str_detect(Var1, "tumor") & Freq > 50) 

group_assignment <- CNV_metrics %>% filter(group %in% filter$Var1) %>% mutate(CIN_status = ifelse(heterogeneity_score > 0.35 & aneuploidy_score > 0.03, "CIN_high", "CIN_low"))
...
```{r}
group_assignment_medf <- CNV_metrics_medf  %>% filter(group %in% filter$Var1) %>% mutate(CIN_status = ifelse(heterogeneity_score > 0.1 & aneuploidy_score > 0.01, "CIN_high", "CIN_low"))
