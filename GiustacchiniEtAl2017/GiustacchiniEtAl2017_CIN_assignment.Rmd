...
```{r}
group_assignment <- CNV_metrics %>% mutate(CIN_status = ifelse(heterogeneity_score > 0.22 & aneuploidy_score > 0.04, "CIN_high", "CIN_low")) %>% filter(!group %in% c("normal"))
...
```{r}
group_assignment_medf <- CNV_metrics_medf %>% mutate(CIN_status = ifelse(heterogeneity_score > 0.027 & aneuploidy_score > 0.011, "CIN_high", "CIN_low"))
