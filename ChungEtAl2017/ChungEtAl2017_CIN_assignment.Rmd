...
```{r}
group_assignment <- CNV_metrics %>% filter(str_detect(group, "Tumor")) %>% mutate(CIN_status = ifelse(heterogeneity_score > 0.4 & aneuploidy_score > 0.05, "CIN_high", "CIN_low"))
...
```{r}
group_assignment_medf <- CNV_metrics_medf  %>% filter(str_detect(group, "Tumor")) %>% mutate(CIN_status = ifelse(heterogeneity_score > 0.19 | aneuploidy_score > 0.045, "CIN_high", "CIN_low"))
