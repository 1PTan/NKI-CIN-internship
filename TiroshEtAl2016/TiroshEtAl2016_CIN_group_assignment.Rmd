---
title: "CIN group assignment Tirosh et al 2016 scRNA-seq"
...
```{r}
group_assignment <- CNV_metrics %>% filter(!group %in% ("non_tumor")) %>% mutate(CIN_status = ifelse(heterogeneity_score > 0.32 & aneuploidy_score > 0.04, "CIN_high", "CIN_low"))
...
```{r}
group_assignment_medf <- CNV_metrics_medf %>% filter(!group %in% ("non_tumor")) %>% mutate(CIN_status = ifelse(heterogeneity_score > 0.125 & aneuploidy_score > 0.03, "CIN_high", "CIN_low"))
...
group_assignment_c %>% filter(CIN_status != CIN_status.medf)
