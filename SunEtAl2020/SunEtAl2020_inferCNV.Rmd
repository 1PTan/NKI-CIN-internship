...
scSun_n <- readRDS(file = "output_sc/scSun_n.rds")
...
# Get gene features
```{r}
genes <- rownames(scSun_n) %>% as.data.frame() %>% rename(gene = ".")
...
```{r}
count_matrix <- GetAssayData(scSun_n, slot = "counts")
...
scSun_metadata <- scSun_n@meta.data

colnames(scSun_metadata)

table(scSun_metadata$cell_type)

annot_scRNA <- scSun_metadata %>% select(cell_type, orig.ident) %>% mutate(
...
# annot_scRNA %>% write.csv2(file = paste(source.output.dir, "annot_scRNA.csv"))
# gene_features.f %>% write.csv2(file = paste(source.output.dir, "gene_features.f.csv"))

annot_scRNA <- read.csv2(file = paste(source.output.dir, "annot_scRNA.csv"))
gene_features.f <- read.csv2(file = paste(source.output.dir, "gene_features.f.csv"))

gene_features.f <- gene_features.f %>% column_to_rownames("X")
annot_scRNA <- annot_scRNA %>% column_to_rownames("X")

```

```{r}
infercnv_scRNA <- infercnv::CreateInfercnvObject(raw_counts_matrix = count_matrix, gene_order_file = gene_features.f, annotations_file = annot_scRNA %>% select(annotation),
