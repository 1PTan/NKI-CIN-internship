...
df.counts.raw <- read.csv("data/GSE98183_Counts.geneSymbols.101bpSE.csv")
...
if(export){
  ...
  source.output.dir <- file.path("output_SE101bp")
...
if (export){
  ...
  # Export data
  df.deseq.results_map %>% write_csv2("data/bulkSE_transcriptomics_deseq.csv")
  
  # DE genes using symmetrical threshold
  df.deseq.results_map %>% filter(within_threshold == TRUE) %>% write_csv2(paste0("data/SE_CIN-diff_deseq_DE_", alpha, "_FC", log2FCthresh, ".csv"))
  ...
  df.deseq.results_map %>% filter(padj < alpha_up & log2FoldChange > log2FCthresh_up) %>% select(gene, gene_name, symbol) %>% write_csv2(paste0("data/SE_CIN-high-up_", alpha_up, "_FC_", log2FCthresh_up, "_n", n_genes_up, ".csv"), quote = FALSE)
  ...
  df.deseq.results_map %>% filter(padj < alpha_down & log2FoldChange < log2FCthresh_down) %>% select(gene, gene_name, symbol) %>% write_csv2(paste0("data/SE_CIN-high-down_", alpha_down, "_FC_", log2FCthresh_down, "_n", n_genes_down, ".csv"), quote = FALSE)
}
```
...
if(export){
  ...
  norm.pseudocounts.df %>% write.table(file ="data/SE_GSEA_CIN_high_low_pseudo.txt", sep = "\t", row.names = FALSE, quote = FALSE)
  ...
  df.deseq.results_map %>% arrange(-log2FoldChange) %>% mutate(gene_symbol = ifelse(is.na(symbol), gene, symbol)) %>% select(gene_symbol, log2FoldChange) %>% dplyr::rename("NAME" = gene_symbol) %>% write.table(file ="data/SE_GSEA_preranked_CIN_high_low.rnk", sep = "\t", row.names = FALSE, quote = FALSE)
...
if (export){
png(filename="data/fGSEA_bulkSE_results_shrunkfc.png", width=800, height=500)
ggsave(file="data/fGSEA_bulkSE_results_shrunkfc.png", width=8, height=5, dpi=300)

fgseaRes %>% select(-leadingEdge) %>% write.csv2("data/bulkSE_fgsea_shrunkfc_CIN_highlow.csv", row.names = FALSE)
}

# fgseaRes <- read.csv2("data/bulk_fgsea_shrunkfc_CIN_highlow.csv")
```
...
if (export){
png(filename="data/fGSEA_bulkSE_results_fc.png", width=800, height=500)
ggsave(file="data/fGSEA_bulkSE_results_fc.png", width=8, height=5, dpi=300)

fgseaRes.noLFC %>% select(-leadingEdge) %>% write.csv2("data/bulkSE_fgsea_notshrunkfc_CIN_highlow.csv", row.names = FALSE)
}


# fgseaRes.noLFC <- read.csv2("data/bulk_fgsea_notshrunkfc_CIN_highlow.csv")
```
...
if (export){
png(filename="data/fGSEA_bulkSE_results_Zstat.png", width=800, height=500)
ggsave(file="data/fGSEA_bulkSE_results_Zstat.png", width=8, height=5, dpi=300)

fgseaRes.noLFC.stat %>% select(-leadingEdge) %>% write.csv2("data/bulkSE_fgsea_Zstat_CIN_highlowcsv", row.names = FALSE)
}

# fgseaRes.noLFC.stat <- read.csv2("data/bulk_fgsea_Zstat_CIN_highlow.csv")
```
