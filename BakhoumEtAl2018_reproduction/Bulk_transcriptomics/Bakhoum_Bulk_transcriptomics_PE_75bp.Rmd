...
# install.packages("BiocManager", "broom", "patchwork", "ggfortify", "colorspace", "GGally", "ggrepel", AnnotationDbi", "tidyverse", "gplots", "pheatmap", "RColorBrewer", "msigdbr", "readxl")

# BiocManager::install("DESeq2", "apeglm", "fgsea", "recount3", "org.Hs.eg.db")
...
# df.counts.raw2 <- read.csv("data/GSE98183_Counts.geneSymbols.csv")
...
# df.counts.raw[df.counts.raw %>% column_to_rownames("gene") %>% rowMeans() > 0,]
...
fgseaRes <- fgseaRes %>% mutate(within_threshold = padj < 0.3, "Enriched_in" = ifelse(NES > 0, "CIN-high", "CIN-low")) %>% left_join(msigdb_hallmarks_names)

fgseaRes %>% select(padj) %>% summary()
...
fgseaRes %>% mutate(padj = ifelse(padj < 0.00001, 0.0000101, padj)) %>% ggplot(aes(x = log10(padj), y = NES)) +
  geom_point(aes(size = 0.5, alpha = 0.3, col = Enriched_in)) + 
  geom_hline(yintercept = 0, col = "black") +
  geom_text_repel(
  aes(x = log10(padj), y = NES, label = name), data = fgseaRes %>% filter(within_threshold), colour = "grey20", force = 3, force_pull = 2, min.segment.length = 0, max.overlaps = 20
  ) +
...
fgseaRes.noLFC.stat %>% ggplot(aes(x = log10(padj), y = NES)) +
  ...
  geom_hline(yintercept = 0, col = "black") +
  geom_text_repel(
  aes(x = log10(padj), y = NES, label = name), data = fgseaRes.noLFC.stat %>% filter(within_threshold), colour = "grey20", force = 3, force_pull = 2, min.segment.length = 0, max.overlaps = 20
  ) +
  labs(x = "log10(FDRq)", y = "Normalized Enrichment Score", col = "Enriched in") + 
...
if(export){
  #png(filename = paste0(source.output.dir,"/GSEA_results.png"), width=600, height=600)
  ggsave(file = paste0(source.output.dir,"/orig_GSEA_results.png"), width=6, height=4, dpi=300)
}
...
if(export){
  #png(filename=paste0(source.output.dir,"/GSEA_x_orig.png"), width=600, height=600)
  ggsave(file=paste0(source.output.dir,"/GSEA_FDR_x_orig.png"), width=6, height=4, dpi=300)
}
```
...
fGSEA_comp <- Bulk_gsea %>% select(NAME, NES, FDR.q.val) %>% rename(orig_NES = NES, orig_FDR.q.val = FDR.q.val) %>% left_join(
  fgseaRes %>% select(pathway, NES, pval, padj) %>% rename(NAME = pathway)
  ) %>% 
  left_join(
  msigdb_hallmarks_names %>% rename(NAME = pathway)
  ) %>% mutate(sign_agreement = sign(orig_NES) == sign(NES))
...
# fGSEA x Bulk
fGSEA_comp <- Bulk_gsea %>% select(NAME, NES, FDR.q.val) %>% rename(orig_NES = NES, orig_FDR.q.val = FDR.q.val) %>% left_join(
  fgseaRes.noLFC %>% select(pathway, NES, pval, padj) %>% rename(NAME = pathway)
  ) %>% 
  left_join(
  msigdb_hallmarks_names %>% rename(NAME = pathway)
  ) %>% mutate(sign_agreement = sign(orig_NES) == sign(NES))
...
colnames(sampleDistMatrix) <- c("dnMCAK","Kif2a","Control","MCAK","Kif2b")
colors <- colorRampPalette( rev(brewer.pal(9, "Blues")) )(255)
heatmap <- pheatmap(sampleDistMatrix,
         clustering_distance_rows=sampleDists,
         clustering_distance_cols=sampleDists,
         col=colors, legend = FALSE, treeheight_row = 0,
         treeheight_col = 50, fontsize =16)

save_pheatmap_png <- function(x, filename, width=800, height=800, res = 300) {
...
if(export){
save_pheatmap_png(heatmap, filename = paste0(source.output.dir, "/sample_clustering.png"), width=800, height=900)
}
...
plotPCA(vsd, intgroup=c("Group")) +
geom_text_repel(
  aes(x = PC1, y = PC2, label = name), data = plotPCA(vsd, intgroup=c("Group"), returnData = TRUE) %>% mutate(name = c("dnMCAK","Kif2a","Control","MCAK","Kif2b")))
...
```{r}
df.deseq.results_map <- read_csv2(file = paste0(source.output.dir, "/bulk_transcriptomics_deseq.csv"))
...
log2FCthresh = 3
#df.volcano <- df.deseq.results %>% mutate(threshold = (abs(log2FoldChange) > 2 & padj < alpha))
df.deseq.results_map <- df.deseq.results_map %>% mutate(within_threshold = abs(log2FoldChange) >= log2FCthresh & pvalue <= alpha)
             
df.deseq.results.noLFC <- df.deseq.results.noLFC %>% mutate(within_threshold = abs(log2FoldChange) >= log2FCthresh & pvalue <= alpha)

# All genes are present
CIN_signature %in% df.deseq.results_map$gene

# Genes in volcano plot quadrant
CIN_signature %in% (df.deseq.results_map %>% filter(within_threshold))$gene

CIN_signature[!CIN_signature %in% (df.deseq.results_map %>% filter(within_threshold))$gene]

CIN_signature[!CIN_signature %in% (df.deseq.results.noLFC %>% filter(within_threshold))$gene]
...
df.deseq.results.noLFC %>% filter(gene == "SEMA6D")

df.deseq.results.noLFC %>% ggplot(
  aes(x=log2FoldChange , y= -log10(pvalue + 1e-200), col = within_threshold)) +     geom_point(alpha = 0.5) +
geom_vline(xintercept = 0, col = "black") + 
geom_vline(xintercept = c(-log2FCthresh, log2FCthresh), col = "red") +   geom_hline(yintercept = -log10(alpha), colour = "red") + 
geom_text_repel(
  aes(x = log2FoldChange, y = -log10(pvalue + 1e-200), label = gene), data = df.deseq.results.noLFC %>% filter(within_threshold), colour = "grey20", min.segment.length = 0
  ) +
  theme(legend.position = "none", plot.title = element_text(hjust= 0.5)) +
  labs(
    title = "Volcano plot", 
...
if (export){
  #png(filename = paste0(source.output.dir, "/volcano_plot_strict.png"), width=600, height=600)
  ggsave(file = paste0(source.output.dir, "/volcano_plot_noLFC_strict.png"), width=6, height=4, dpi=300)
}

df.deseq.results.noLFC %>% filter(within_threshold, log2FoldChange > 0)
```

# Original data
```{r}
df.deseq.results.orig <- read_excel(paste0(source.output.dir, "/MS_CINHighvsLow.xlsx"))

# This file turned out to be FPKM data, not the normalized counts from DESEq
raw_data_df <- read_excel("data/Raw-Data.xlsx")

colnames(raw_data_df)
colnames(raw_data_df) <- c("gene","MK_orig","Kb_orig","MKH_orig","cont_orig","Ka_orig")
...
deseq.c <- df.deseq.results_map %>% left_join(df.deseq.results.orig, by = "gene")

deseq.c %>% ggplot(aes(x = log2FoldChange, y = orig_log2FoldChange)) + geom_point() + geom_abline(slope = 1)

deseq.noLFC.c %>% ggplot(aes(x = log2FoldChange, y = orig_log2FoldChange)) + geom_point() + geom_abline(slope = 1)

if (export){
#png(filename = paste0(source.output.dir,"/fGSEA_bulk_results_fc.png"), width=800, height=500)
ggsave(file = paste0(source.output.dir,"/not_shrunkLFC_x_orig.png"), width=6, height=4, dpi=300)
}
```

```{r}
deseq.norm.c <- deseq.dataset.norm %>% rownames_to_column("gene") %>% inner_join(raw_data_df, by = "gene")

# df.counts.raw %>% inner_join(deseq.norm.orig, by = "gene")

deseq.norm.c %>% ggplot(aes(y = MKH, x = MKH_orig)) + geom_point() + geom_abline(slope = 1) + 
deseq.norm.c %>% ggplot(aes(y = MK, x = MK_orig)) + geom_point() + geom_abline(slope = 1) + 
  deseq.norm.c %>% ggplot(aes(y = Kb, x = Kb_orig)) + geom_point() + geom_abline(slope = 1) +
  deseq.norm.c %>% ggplot(aes(y = Ka, x = Ka_orig)) + geom_point() + geom_abline(slope = 1) +
  deseq.norm.c %>% ggplot(aes(y = cont, x = cont_orig)) + geom_point() + geom_abline(slope = 1)
```

```{r}
df.counts.FPKM <- read.csv("data/GSE98183_FPKM.geneSymbols.75bpPE.csv")
colnames(df.counts.FPKM)
df.counts.FPKM <- df.counts.FPKM %>% rename("gene" = X)

sum(raw_data_df$gene %in% df.counts.FPKM$gene)

raw_data_df[!raw_data_df$gene %in% df.counts.FPKM$gene, ] %>% head()

df.FPKM.c <- raw_data_df %>% inner_join(df.counts.FPKM, by = "gene")
df.FPKM.c

# FPKM counts are the same, but there are only 23276 genes in the raw data file. Out of these genes, only 17450 have matching names with the FPKM file. Most of the ones who do not match are ensembl IDs

df.FPKM.c %>% ggplot(aes(y = MKH, x = MKH_orig)) + geom_point() + geom_abline(slope = 1) + 
df.FPKM.c %>% ggplot(aes(y = MK, x = MK_orig)) + geom_point() + geom_abline(slope = 1) + 
  df.FPKM.c %>% ggplot(aes(y = Kb, x = Kb_orig)) + geom_point() + geom_abline(slope = 1) +
  df.FPKM.c %>% ggplot(aes(y = Ka, x = Ka_orig)) + geom_point() + geom_abline(slope = 1) +
  df.FPKM.c %>% ggplot(aes(y = cont, x = cont_orig)) + geom_point() + geom_abline(slope = 1)
```

# FGSEA original
```{r}
CIN_fc_rnk_orig <- df.deseq.results.orig %>% select(gene, orig_log2FoldChange) %>% deframe()

fgseaRes.orig <- fgsea(pathways = msigdb_hallmarks_set, 
                  stats    = CIN_fc_rnk_orig,
                  minSize  = 15,
                  maxSize  = 500)

fgseaRes.orig <- fgseaRes.orig %>% mutate(within_threshold = padj < 0.05, "Enriched_in" = ifelse(NES > 0, "CIN-high", "CIN-low")) %>% left_join(msigdb_hallmarks_names)

# fgseaRes.orig %>% select(NES,padj) %>% summary()

fgseaRes.orig %>% mutate(padj = ifelse(padj < 0.001, 0.00101, padj)) %>% ggplot(aes(x = log10(padj), y = NES)) +
  geom_point(aes(size = 0.5, alpha = 0.3, col = Enriched_in)) + 
  geom_hline(yintercept = 0, col = "black") +
  geom_text_repel(
  aes(x = log10(padj), y = NES, label = name), data = fgseaRes.orig %>% filter(within_threshold) %>% mutate(padj = ifelse(padj < 0.001, 0.00101, padj)), colour = "grey20", force = 3, force_pull = 2, min.segment.length = 0, max.overlaps = 20
  ) +
  labs(x = "log10(FDRq)", y = "Normalized Enrichment Score", col = "Enriched in") + 
  scale_y_continuous(limits = c(-2,2.5), breaks = seq(-2,2.5,1)) + 
  scale_x_continuous(trans = "reverse", limits = c(0, -3), breaks = c(-3, -2, -1, 0) , labels = c("<0.001", "0.01", "0.1", "1")) + scale_fill_manual(values = c("red","blue")) + guides(alpha = FALSE, size = FALSE)
...
if (export){
ggsave(file = paste0(source.output.dir,"/fGSEA_bulk_results_orig_fc.png"), width=8, height=5, dpi=300)
  
fgseaRes.orig %>% select(-leadingEdge) %>% write.csv2("fgseaRes.orig.csv")
}
...
```{r}
# fGSEA x original GSEA
fGSEA_orig_comp <- Bulk_gsea %>% select(NAME, NES, FDR.q.val) %>% rename(orig_NES = NES, orig_FDR.q.val = FDR.q.val) %>% left_join(
  fgseaRes.orig %>% select(pathway, NES, pval, padj) %>% rename(NAME = pathway)
  ) %>% 
  left_join(
  msigdb_hallmarks_names %>% rename(NAME = pathway)
  ) %>% mutate(sign_agreement = sign(orig_NES) == sign(NES))

fGSEA_orig_comp %>% ggplot(aes(x = orig_NES, y = NES, col = log10(padj), shape = padj < 0.2)) + 
  geom_point() + geom_abline(slope = 1) + 
  geom_text_repel(
    aes(x = orig_NES, y = NES, label = name), data = fGSEA_orig_comp %>% filter(!sign_agreement), colour = "grey20", force = 3, force_pull = 2, min.segment.length = 0, max.overlaps = 20
  ) + labs(x = "Orig GSEA NES", y = "fGSEA NES") +
  scale_color_gradient2(low="black", mid = "blue", high="red", midpoint = -1)

if (export){
ggsave(file = paste0(source.output.dir,"/fGSEA_orig_bulk_x_orig.png"), width=8, height=5, dpi=300)
}

# Compare GSEA FDRq values
fGSEA_orig_comp %>% ggplot(aes(x=orig_FDR.q.val, y= padj, col = NES)) + geom_point() + geom_abline(slope = 1)  + labs(x = "orig_FDR.q.val", y = "FDR.q.val") +
  scale_color_gradient2(low="blue", mid = "white", high="red", midpoint = 0)

if(export){
  ggsave(file=paste0(source.output.dir,"/origfGSEA_x_orig.fdr.png"), width=6, height=4, dpi=300)
}
```
# Recount3 data
```{r}
...
proj_info <- subset(
    human_projects,
    project == "SRP105199" & project_type == "data_sources"
)

rse_gene_SRP105199 <- create_rse(proj_info)

assay(rse_gene_SRP105199, "counts") <- transform_counts(rse_gene_SRP105199)

assay(rse_gene_SRP105199)
colnames(assay(rse_gene_SRP105199))

rse_gene_SRP105199@colData$sra.experiment_title
rse_gene_SRP105199@colData$sra.experiment_acc

# These are only the 41 SE samples, not including the parental cell lines (SE or PE) 
```
