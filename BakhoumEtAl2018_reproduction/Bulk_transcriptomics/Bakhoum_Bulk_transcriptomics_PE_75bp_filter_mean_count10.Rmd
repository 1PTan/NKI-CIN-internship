...
export = FALSE
source.output.dir <- file.path("output_PE75bp_filter_mean_10_orig")
...
df.counts.long %>%
  group_by(gene) %>% filter(any(Count > 0)) %>% ungroup() %>% select(gene) %>% n_distinct()

# df.counts.long %>%
#   group_by(gene) %>% summarize(mean = mean(Count)) %>% ungroup() %>% filter(mean >= 10)

df.counts.long <- df.counts.long %>% left_join(df.counts.long %>%
  group_by(gene) %>% summarize(mean = mean(Count)))

# filter out genes which have mean counts below 10.
# Also create a column for log2 of pseudocounts
df.counts.f <- df.counts.long %>% filter(mean >= 10) %>% 
  mutate(counts_log2 = log2(Count+1))
...
```{r}
#?lfcShrink
resLFC <- lfcShrink(dds, coef = "Group_CIN.high_vs_CIN.low", type = "normal")

resLFCapeglm <- lfcShrink(dds, coef = "Group_CIN.high_vs_CIN.low", type = "apeglm")
...
df.deseq.resultsapeglm <- resLFCapeglm %>%
  as.data.frame() %>%
  rownames_to_column("gene")

df.deseq.results %>% head()
df.deseq.resultsapeglm %>% head()
...
```
```{r}
df.deseq.results %>% left_join(df.deseq.resultsapeglm %>% select(gene, log2FoldChange), suffix = c(".normal", ".apeglm"), by= "gene") %>% ggplot(aes(x = log2FoldChange.normal, y = log2FoldChange.apeglm)) + geom_point() + geom_abline(slope = 1)
```
...
df.deseq.results.noLFC %>% select(gene, stat, pvalue) %>% left_join(
  df.deseq.results %>% select(gene, log2FoldChange)
) %>% ggplot(aes(x = stat/5, y = log2FoldChange, col= pvalue)) + 
  geom_point() + geom_abline(slope = 1, alpha = 0.5) +
  labs(x = "1/5 Z-statistic statistic", y = "Shrunk LFC")# + coord_flip()
...
fgseaRes %>% mutate(padj = ifelse(padj < 0.001, 0.00101, padj)) %>% ggplot(aes(x = log10(padj), y = NES)) +
  ...
  geom_text_repel(
  aes(x = log10(padj), y = NES, label = name), data = fgseaRes %>% filter(within_threshold | name %in% c("EMT", "Inflammatory Response", "IFN-a Response", "IFN-y Response", "TNF-a Signaling via NKFB", "IL2-STAT5 Signaling", "KRAS Signaling Up")), colour = "grey20", force = 3, force_pull = 2, min.segment.length = 0, max.overlaps = 20
  ) +
...
ggsave(file = paste0(source.output.dir,"/fGSEA_bulk_results_shrunkfc.png"), width=6, height=4, dpi=300)
...
fgseaRes.noLFC %>% ggplot(aes(x = log10(padj), y = NES)) +
  ...
  geom_hline(yintercept = 0, col = "black") +
  geom_text_repel(
  aes(x = log10(padj), y = NES, label = name), data = fgseaRes.noLFC %>% filter(within_threshold | name %in% c("EMT", "Inflammatory Response", "IFN-a Response", "IFN-y Response", "TNF-a Signaling via NKFB", "IL2-STAT5 Signaling", "KRAS Signaling Up")), colour = "grey20", force = 3, force_pull = 2, min.segment.length = 0, max.overlaps = 20
  ) +
  labs(x = "log10(FDRq)", y = "Normalized Enrichment Score", col = "Enriched in") + 
  scale_y_continuous(limits = c(-2,2), breaks = seq(-2,2,1)) + 
...
fgseaRes.noLFC.stat %>% ggplot(aes(x = log10(padj), y = NES)) +
  ...
  geom_hline(yintercept = 0, col = "black") +
  geom_text_repel(
  aes(x = log10(padj), y = NES, label = name), data = fgseaRes.noLFC.stat %>% filter(within_threshold | name %in% c("EMT", "Inflammatory Response", "IFN-a Response", "IFN-y Response", "TNF-a Signaling via NKFB", "IL2-STAT5 Signaling", "KRAS Signaling Up")), colour = "grey20", force = 3, force_pull = 2, min.segment.length = 0, max.overlaps = 20
  ) +
  labs(x = "log10(FDRq)", y = "Normalized Enrichment Score", col = "Enriched in") + 
...
fGSEA_comp %>% ggplot(aes(x = orig_NES, y = NES, col = log10(padj), shape = padj < 0.2)) + 
  ...
  geom_text_repel(
    aes(x = orig_NES, y = NES, label = name), data = fGSEA_comp %>% filter(!sign_agreement | name %in% c("EMT", "Inflammatory Response", "IFN-a Response", "IFN-y Response", "TNF-a Signaling via NKFB", "IL2-STAT5 Signaling", "KRAS Signaling Up")), colour = "grey20", force = 3, force_pull = 2, min.segment.length = 0, max.overlaps = 20
...
fGSEA_comp %>% ggplot(aes(x=orig_FDR.q.val, y= padj, col = NES)) + geom_point() + geom_abline(slope = 1)  + labs(x = "orig_FDR.q.val", y = "FDR.q.val") +
  ...
  geom_text_repel(
    aes(x = orig_FDR.q.val, y = padj, label = name), data = fGSEA_comp %>% filter(!sign_agreement ), colour = "grey20", force = 3, force_pull = 2, min.segment.length = 0, max.overlaps = 20
  ) 

if(export){
  ggsave(file=paste0(source.output.dir,"/fGSEA_x_orig.fdr.png"), width=6, height=4, dpi=300)
}
```
...
if(export){
save_pheatmap_png(heatmap, filename = paste0(source.output.dir, "/sample_clustering.png"))
}
...
deseq.norm.orig <- read_excel("data/Raw-Data.xlsx")
...
deseq.c <- df.deseq.results %>% left_join(df.deseq.results.orig, by = "gene")

deseq.c %>% ggplot(aes(x = log2FoldChange, y = orig_log2FoldChange)) + geom_point() + geom_abline(slope = 1) + labs(x = "normal Log2FC", y = "original Log2FC")

if (export){
#png(filename = paste0(source.output.dir,"/fGSEA_bulk_results_fc.png"), width=800, height=500)
ggsave(file = paste0(source.output.dir,"/shrunkLFC_x_orig.png"), width=6, height=4, dpi=300)
}
...
deseq.glm.c <- df.deseq.resultsapeglm %>% left_join(df.deseq.results.orig, by = "gene")

deseq.glm.c %>% ggplot(aes(x = log2FoldChange, y = orig_log2FoldChange)) + geom_point() + geom_abline(slope = 1) + labs(x = "apeglm Log2FC", y = "original Log2FC")
if (export){
#png(filename = paste0(source.output.dir,"/fGSEA_bulk_results_fc.png"), width=800, height=500)
ggsave(file = paste0(source.output.dir,"/apeglm_shrunkLFC_x_orig.png"), width=6, height=4, dpi=300)
}
```
# FGSEA original
```{r}
