---
title: "Bakhoum et al 2018 scRNA-seq analysis"
...
In this report, the expensive computations are commented out. We plot results from the loaded Seurat objects instead
...
## Clustering cells
```{r}
# scRNA_cn <- FindNeighbors(scRNA_cn, dims = 1:200)
...
# Plotting EMT signature  from Bakhoum et al 2018
...
# Signal is not so clear, but clusters 3 and 4 appear to correspond somewhat with subpopulation "M"
```
...
# Plot CIN high x cin low pseudocounts in log1p space
...
# cluster.M <- read.csv2("data/scRNA_clusterM.csv")
```
...
DE_comp <- avg.CINhigh.cells %>% full_join(avg.CINlow.cells, by = "gene") %>% left_join(
  CIN.DE %>% mutate(CIN_DE = p_val_adj < 0.001, label = p_val_adj < 1e-260)  %>% select(gene, CIN_DE, label)
)
...
# GSEA results based on signed inverse of p-value
...
## Population "M" FGSEA
```{r}
M_fgseaRes <- read.csv2("data/scRNA_fgsea_M.csv")
...
#     cluster.DE.fgsea[[i]] %>% mutate(cluster = i)
#     )
# }
...
cluster.DE.GObp <- cluster.DE.GObp_summary %>% split(f = cluster.DE.GObp_summary$cluster)
...
#     cluster.DE.GObp[[i]] %>% summary() %>% mutate(cluster = i)
#     )
# # }
# 
# if(export){
...
```{r}
scRNA_regcycle <- readRDS("data/Bakhoum_scRNA_cellreg.rds")
...
# scRNA_regcycle <- ScaleData(scRNA_regcycle, vars.to.regress = vars_to_regress)
...
# cellreg.CIN.DE <- FindMarkers(scRNA_regcycle, ident.1 = "CIN-high", min.pct = 0.05, logfc.threshold = 0) %>% rownames_to_column("gene")
...
#cellreg.CIN.DE %>% filter(str_detect(gene, "BMP2"))
```
...
cellreg.cluster.DE.fgsea_summary %>% left_join(msigdb_hallmarks_names) %>% select(name, NES, cluster) %>% pivot_wider(values_from = NES, names_from = name) %>% column_to_rownames("cluster") %>% mutate_all(~replace(., is.na(.), 0)) %>% as.matrix() %>% pheatmap(color = colfunc(10))
...
## GO BP Enrichment
```{r}
cellreg.cluster.DE.GObp_summary <- read.csv2("data/scRNA_cellreg_cluster_GObp.csv")
cellreg.cluster.DE.GObp <- cellreg.cluster.DE.GObp_summary %>% split(cellreg.cluster.DE.GObp_summary$cluster)
...
#   cellreg.cluster.DE.GObp[[i]] %>% select()
#   
# }

cellreg.cluster.DE.GObp_summary %>% select(Term, Pvalue, cluster)
...
table(scRNA_SCT_cellreg$clusterID, scRNA_SCT_cellreg$CIN_status) %>% as.data.frame() %>% rename("clusterID" = Var1, "group" = Var2) %>% ggplot(aes(x = clusterID, y = group, fill = Freq)) + geom_tile() + scale_fill_distiller(palette = "PuBu") + geom_text(aes(label=Freq)) + labs(y = "", fill = "Number of cells")
...
scRNA_norm_cn <- readRDS(file = "data/Bakhoum_scRNA_norm_cn.rds")
...
ElbowPlot(scRNA_norm_cn, ndims = 50)
...
```{r}
IntegratedCIN <- readRDS("data/Bakhoum_integrated_scRNA.rds")
