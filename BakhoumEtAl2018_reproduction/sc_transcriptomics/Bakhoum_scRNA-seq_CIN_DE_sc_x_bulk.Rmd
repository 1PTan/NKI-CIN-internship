...
```{r}
CIN.DE.c <- CIN.DE.bulk %>% inner_join(CIN.DE %>% rename("symbol" = gene), by = "symbol")  %>% left_join(sc_means)

CIN.DE.c %>% ggplot(aes(x = log2FoldChange, y = avg_log2FC, size = sc_mean, col = -log10(p_val))) + geom_point(alpha = 0.8) + geom_abline(slope = 1, col = "blue", alpha = 0.5) + labs(title = "CIN-High x CIN-Low Log2FC", x = "Bulk transcriptomics", y = "scRNA-seq transcriptomics", col = "-log10(sc.p.value)") + geom_text_repel(
  aes(x = log2FoldChange, y = avg_log2FC, size = 1, label = symbol), data = CIN.DE.c %>% filter(gene %in% CIN_signature), colour = "grey20", force = 3, force_pull = 2, min.segment.length = 0, max.overlaps = 20
...
```{r}
# Signed P value instead of log2FC
CIN.DE.c <- CIN.DE.c %>% mutate(bulk_sign = sign(log2FoldChange), sc_sign = sign(avg_log2FC))

CIN.DE.c %>% ggplot(aes(x = -log(pvalue) * bulk_sign, y = -log(p_val) * sc_sign, col = sc_mean, size = abs(avg_log2FC))) + geom_point(alpha = 0.8) + geom_abline(slope = 1, col = "blue", alpha = 0.5) + labs(title = "CIN-High x CIN-Low signed -log(p-value)", x = "Bulk transcriptomics", y = "scRNA-seq transcriptomics") + geom_text_repel(
  aes(x = -log(pvalue) * bulk_sign, y =  -log(p_val) * sc_sign, size = 1, label = symbol), data = CIN.DE.c %>% filter(p_val < 1e-15), colour = "grey20", force = 3, force_pull = 2, min.segment.length = 0, max.overlaps = 20
  ) + ylim(-500, 500) + xlim(-100, 100) + theme(plot.title = element_text(hjust= 0.5)) + scale_colour_gradient(low = "gray", high = "blue")

cor(CIN.DE.c$pvalue * CIN.DE.c$bulk_sign, CIN.DE.c$p_val * CIN.DE.c$sc_sign)
cor(-log(CIN.DE.c$pvalue + 1e-300) * CIN.DE.c$bulk_sign, -log(CIN.DE.c$p_val  + 1e-300) * CIN.DE.c$sc_sign)
```
# Compare with lower pseudocounts 0.001
```{r}
CIN.DE.c2 <- CIN.DE.bulk %>% inner_join(CIN.DE2 %>% rename("symbol" = gene)) %>% mutate(bulk_sign = sign(log2FoldChange), sc_sign = sign(avg_log2FC))  %>% left_join(sc_means)

CIN.DE.c2 %>% 
  ggplot(aes(x = log2FoldChange, y = avg_log2FC, size = sc_mean, col = -log10(p_val))) +
  ...
  labs(title = "CIN-High x CIN-Low Log2FC", x = "Bulk transcriptomics", y = "scRNA-seq transcriptomics") + 
  geom_text_repel(
  aes(x = log2FoldChange, y = avg_log2FC, size = 1, label = symbol), data = CIN.DE.c2 %>% filter(within_threshold), colour = "grey20", force = 3, force_pull = 2, min.segment.length = 0, max.overlaps = 15
  ) + theme(plot.title = element_text(hjust = 0.5)) + scale_color_gradient2(low="red", mid = "gray", high="blue", midpoint = 50)

if(export){
  #png(filename=paste0(source.output.dir,"/GSEA_x_orig.png"), width=600, height=600)
  ggsave(file=paste0(source.output.dir,"/pseudo0.001_sc_x_bulk.png"), width=6, height=4, dpi=300)
}

cor(CIN.DE.c2$log2FoldChange, CIN.DE.c2$avg_log2FC)

# Check agreement between the sign of genes
table(sign(CIN.DE.c2$log2FoldChange), sign(CIN.DE.c2$avg_log2FC))
prop.table(table(sign(CIN.DE.c2$log2FoldChange), sign(CIN.DE.c2$avg_log2FC)))
```

# Compare with lower pseudocounts 0.001 cutoff
```{r}
# Filter genes with pct expression below 5%
CIN.DE.c2.cutoff <- CIN.DE.bulk %>% inner_join(CIN.DE2.cutoff %>% rename("symbol" = gene)) %>% mutate(bulk_sign = sign(log2FoldChange), sc_sign = sign(avg_log2FC)) %>% left_join(sc_means)
...
CIN.DE.c2.cutoff %>% 
  ...
  labs(title = "CIN-High x CIN-Low Log2FC", x = "Bulk transcriptomics", y = "scRNA-seq transcriptomics") + 
  geom_text_repel(
  aes(x = log2FoldChange, y = avg_log2FC, size = 1, label = symbol), data = CIN.DE.c2.cutoff %>% filter(within_threshold), colour = "grey20", force = 3, force_pull = 2, min.segment.length = 0, max.overlaps = 20
  ) + theme(plot.title = element_text(hjust = 0.5)) + scale_color_gradient2(low="red", mid = "gray", high="blue", midpoint = 50)

if(export){
  #png(filename=paste0(source.output.dir,"/GSEA_x_orig.png"), width=600, height=600)
  ggsave(file=paste0(source.output.dir,"/pseudo0.001_sc_x_bulk_cutoff5.png"), width=6, height=4, dpi=300)
}

cor(CIN.DE.c2.cutoff$log2FoldChange, CIN.DE.c2.cutoff$avg_log2FC)

# Check agreement between the sign of genes
table(sign(CIN.DE.c2.cutoff$log2FoldChange), sign(CIN.DE.c2.cutoff$avg_log2FC))
prop.table(table(sign(CIN.DE.c2.cutoff$log2FoldChange), sign(CIN.DE.c2.cutoff$avg_log2FC)))
```
...
# Compare with Gamma Poison
```{r}
# Gamma poisson GLM DE x bulk 
CIN.DE.c.gampoi <- CIN.DE.bulk %>% inner_join(CIN.DE.glmGamPoi %>% rename("symbol" = gene), by = "symbol") %>% left_join(sc_means)

CIN.DE.c.gampoi.cutoff <- CIN.DE.c.gampoi %>% mutate(lfc = ifelse(abs(lfc) < 1e6, lfc, Inf* sign(lfc)))

CIN.DE.c.gampoi.cutoff %>% ggplot(aes(x = log2FoldChange, y = lfc, col = sc_mean)) + geom_point(alpha = 0.8) + 
  geom_abline(slope = 1, col = "blue", alpha = 0.5) + 
  geom_hline(yintercept = 0, alpha = 0.5) + 
  geom_vline(xintercept = 0, alpha = 0.5) + 
  labs(title = "CIN-High x CIN-Low", x = "Bulk Log2FC", y = "scRNA-seq Log2FC") + 
  geom_text_repel(
  aes(x = log2FoldChange, y = lfc, label = symbol), data = CIN.DE.c.gampoi.cutoff %>% filter(symbol %in% CIN_signature), colour = "grey20", force = 3, force_pull = 2, min.segment.length = 0, max.overlaps = 10
  ) + theme(plot.title = element_text(hjust = 0.5))  + scale_color_gradient2(low ="gray",mid ="gray", high="blue")

if(export){
  ggsave(file=paste0(source.output.dir,"/gampoi_sc_x_bulk_shrunkLFC.png"), width=6, height=4, dpi=300)
}

cor(CIN.DE.c.gampoi$log2FoldChange, CIN.DE.c.gampoi$lfc)
cor((CIN.DE.c.gampoi %>% filter(abs(lfc) < 1e6))$log2FoldChange, (CIN.DE.c.gampoi%>% filter(abs(lfc) < 1e6))$lfc)

# Check agreement between the sign of genes
table(sign(CIN.DE.c.gampoi$log2FoldChange), sign(CIN.DE.c.gampoi$lfc))

round(prop.table(table(sign(CIN.DE.c.gampoi$log2FoldChange), sign(CIN.DE.c.gampoi$lfc))), 3)
```
...
CIN.DE.c.deseqWald %>% ggplot(aes(x = stat.bulk, y = stat.sc, col = sc_mean, size = abs(log2FoldChange.sc))) + geom_point() + 
  ...
  geom_text_repel(
  aes(x = stat.bulk, y = stat.sc, label = symbol, size = 1), data = CIN.DE.c.deseqWald %>% filter(symbol %in% CIN_signature), colour = "grey20", force = 3, force_pull = 2, min.segment.length = 0, max.overlaps = 20
...
cor(CIN.DE.c.deseqWald.cutoff$log2FoldChange.bulk, CIN.DE.c.deseqWald.cutoff$log2FoldChange.sc)
```
...
```{r}
CIN.DE.c2 <- CIN.DE.c2 %>% mutate(sc_bulk_ratio = avg_log2FC/log2FoldChange, bulk_sc_ratio = log2FoldChange/avg_log2FC) %>% left_join(sc_means)

CIN.DE.c2 %>% select(sc_bulk_ratio, bulk_sc_ratio, baseMean, sc_mean) %>% summary()

CIN.DE.c2 %>% ggplot(aes(sc_bulk_ratio)) + geom_histogram(col = "blue", bins = 1000) + labs(title = "CIN-High x CIN-Low", x = "scRNA-seq/Bulk Log2FC transcriptomics", y = "Count")

CIN.DE.c2 %>% ggplot(aes(sc_bulk_ratio)) + geom_histogram(col = "blue", bins = 100) + labs(title = "CIN-High x CIN-Low", x = "scRNA-seq/Bulk Log2FC transcriptomics", y = "Count") + xlim(-10, 10)

```

```{r}
CIN.DE.c2 %>% ggplot(aes(x = sc_bulk_ratio, y = baseMean, col = log(pvalue))) + geom_point()  + geom_text_repel(
  aes(x = sc_bulk_ratio, y = baseMean, label = symbol), data = CIN.DE.c %>% filter(abs(sc_bulk_ratio) > 10), colour = "grey20", force = 3, force_pull = 2, min.segment.length = 0, max.overlaps = 20
  ) + scale_color_gradient2(low="blue", mid = "black", high="red", midpoint = -2)
...
CIN.DE.c2 %>% ggplot(aes(x = bulk_sc_ratio, y = baseMean, col = log(pvalue))) + geom_point()  + geom_text_repel(
  aes(x = bulk_sc_ratio, y = baseMean, label = symbol), data = CIN.DE.c %>% filter(abs(sc_bulk_ratio) > 10), colour = "grey20", force = 3, force_pull = 2, min.segment.length = 0, max.overlaps = 20
  ) + scale_color_gradient2(low="blue", mid = "black", high="red", midpoint = -2)


CIN.DE.c2 %>% filter(within_threshold) %>% ggplot(aes(x = sc_bulk_ratio, y = baseMean, col = sc_mean)) + geom_point()  + geom_text_repel(
  aes(x = sc_bulk_ratio, y = baseMean, label = symbol), data = CIN.DE.c %>% filter(within_threshold), colour = "grey20", force = 3, force_pull = 2, min.segment.length = 0, max.overlaps = 20)
```
...
fgseaRes2 %>% mutate(pval = ifelse(pval < 0.001, 0.00101, pval)) %>% ggplot(aes(x = log10(pval), y = NES)) +
  ...
  geom_hline(yintercept = 0, col = "black") +
  geom_text_repel(
  aes(x = log10(pval), y = NES, label = name), data = fgseaRes2 %>% filter(within_threshold | name %in% c("EMT", "Inflammatory Response", "IFN-a Response", "IFN-y Response", "TNF-a Signaling via NKFB", "IL2-STAT5 Signaling", "KRAS Signaling Up")), colour = "grey20", force = 3, force_pull = 2, min.segment.length = 0
  ) +
