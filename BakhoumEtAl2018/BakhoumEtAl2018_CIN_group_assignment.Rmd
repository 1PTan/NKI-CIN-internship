---
title: "scRNA-seq CIN group assignment"
...
```{r}
group_assignment <- CNV_metrics %>% mutate(CIN_status = ifelse(heterogeneity_score > 0.15 & aneuploidy_score > 0.02, "CIN_high", "CIN_low"))
...
group_assignment %>% 
  ...
  xlim(0,  max(CNV_metrics$heterogeneity_score) + 0.05) + 
  ylim(0, max(CNV_metrics$aneuploidy_score) + 0.01) + 
  labs(x = "Heterogeneity Score", y = "Aneuploidy Score")
 
ggsave(file = file.path(source.output.dir, "CNV_metrics_assignment.png"), width=4, height=2, dpi=300)
...
```{r}
group_assignment_medf <- CNV_metrics_medf %>% mutate(CIN_status = ifelse(heterogeneity_score > 0.02 & aneuploidy_score > 0.01, "CIN_high", "CIN_low"))
