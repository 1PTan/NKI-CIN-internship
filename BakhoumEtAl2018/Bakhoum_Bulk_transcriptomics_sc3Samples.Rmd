...
source.output.dir <- file.path("output_bulkPE75bp_sc3Samples")
...
if (export){
  ggsave(file = file.path(source.output.dir, "volcano_plot_strict.png"), width=6, height=4, dpi=300)
}
...
df.deseq.results.noLFC %>% mutate(
  within_threshold = (padj < 1e-10 & log2FoldChange > 2)
) %>% filter(within_threshold)
...
if (export){
  ggsave(file = file.path(source.output.dir, "volcano_plot_shrunk.png"), width=6, height=4, dpi=300)
}
...
df.deseq.results.shrunk %>% mutate(
  within_threshold = (padj < 1e-5 & log2FoldChange > 2)
) %>% filter(within_threshold)
...
df.deseq.results.shrunk %>% mutate(
  ...
  aes(x=log2FoldChange , y= -log10(padj + 1e-200), col = within_threshold)) + geom_point(alpha = 0.5) +
geom_vline(xintercept = 0, col = "black") + 
geom_vline(xintercept = 2, col = "red") +   geom_hline(yintercept = -log10(1e-5), colour = "red") + 
geom_text_repel(
  aes(x = log2FoldChange, y = -log10(padj + 1e-200), label = gene), data = df.deseq.results.shrunk %>% filter(gene %in% CIN_signature), colour = "grey20", min.segment.length = 0
  ) +
  theme(legend.position = "none", plot.title = element_text(hjust= 0.5)) +
...
fgseaRes.noLFC.stat %>% mutate(padj = ifelse(padj < 0.001, 0.00101, padj)) %>% ggplot(aes(x = log10(padj), y = NES)) +
  geom_point(aes(size = 0.5, alpha = 0.3, col = Enriched_in)) + 
  geom_hline(yintercept = 0, col = "black") +
  geom_text_repel(
  aes(x = log10(padj), y = NES, label = name), data = fgseaRes.noLFC.stat %>% mutate(padj = ifelse(padj < 0.001, 0.00101, padj)) %>% filter(within_threshold | name %in% c("Apical Surface", "MYC Targets V1", "IFN-a Response", "IFN-y Response", "OxPhos", "TGF-B Signaling", "EMT", "IL2-STAT5 Signaling", "Wnt B-Catenin Signaling", "TNF-a Signaling via NKFB", "Inflammatory Response", "KRAS Signaling Up")), colour = "grey20", force = 3, force_pull = 2, min.segment.length = 0, max.overlaps = 20
  ) +
...
if (export){
ggsave(file = file.path(source.output.dir,"fGSEA_bulk_results_Zstat_p.png"), width=7, height=5, dpi=300)
}
...
if(export){
  #png(filename=file.path(source.output.dir,"GSEA_x_orig.png"), width=600, height=600)
...
if(export){
ggsave(file = file.path(source.output.dir,"vst_pca.png"), width=4, height=4, dpi=300)
}
