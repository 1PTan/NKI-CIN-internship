---
title: "CIN group assignment Karaayvaz et al 2018 scRNA-seq"
...
```{r}
group_assignment <- CNV_metrics %>% filter(str_detect(group, "tumor")) %>% mutate(CIN_status = ifelse(heterogeneity_score > 0.45 & aneuploidy_score > 0.05, "CIN_high", "CIN_low"))
...
```{r}
group_assignment_medf <- CNV_metrics_medf %>% filter(str_detect(group, "tumor")) %>% mutate(CIN_status = ifelse(heterogeneity_score > 0.015 & aneuploidy_score > 0.025, "CIN_high", "CIN_low"))
